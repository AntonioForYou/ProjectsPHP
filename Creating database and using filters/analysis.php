    ...
    <div class="container-fluid bg-dark">
        <nav class="navbar container justify-content-center flex-column">
            <div class="row justify-content-md-center">
                ...
                <div class="col-md-auto align-content-center">
                    <a href="#" class="head-link nav-link"> <img src="icons/Enter.svg" alt=""> Войти</a>
                </div>
            </div>
        </nav>
    </div>
...
<div class="container">
    <form action="analysis.php" method="get" class="test-content">
        ...
        <div class="mb-3">
            <label>По цене:</label>
            <input type="number" name="costFrom" placeholder="Цена от" value="<?php if(!empty($arBinds['costFrom'])) echo $arBinds['costFrom'];?>" class="form-control">
            <input type="number" name="costTo" placeholder="Цена до" value="<?php if(!empty($arBinds['costTo'])) echo $arBinds['costTo'];?>" class="form-control mt-3">
        </div>
        <div class="mb-3">
            ...
            <select name="disease" class="form-control">
                ...
                <?php foreach ($disease as $item):?>
                    <option <?php if(!empty($arBinds['disease']) && $arBinds['disease'] == $item['id'])
                        echo 'selected="selected"'?> value="<?=$item['id']?>"><?=$item['name']?></option>
                <?php endforeach;?>
            </select>
        </div>
        <div class="mb-3">
            ...
            <input type="text" name="description" placeholder="Введите описание"
                   value="<?php if(!empty($arBinds['description'])) echo $arBinds['description'];?>" class="form-control">
        </div>
        <div class="mb-3">
            <label>Фильтрация по анализу:</label>
            <input type="text" name="analysis" placeholder="Введите наименование анализа"
                   value="<?php if(!empty($arBinds['analysis'])) echo $arBinds['analysis'];?>" class="form-control">
        </div>
        <div class="row">
        ...
        <table class="table">
            ...
            <tbody>
                <?php foreach ($fullList as $item):?>
                    <tr class="border">
                        <th scope="row"><img src="img/<?=$item['img_path']?>" style="max-width: 250px"></th>
                        <td><?=$item['name']?></td>
                        <td><?=$item['diseaseName']?></td>
                        <td><?=$item['description']?></td>
                        <td class="test"><?=$item['cost']?></td>
                    </tr>
                <?php endforeach;?>
            </tbody>
        </table>
