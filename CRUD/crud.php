<?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/ProjectsPHP/core/logic_crud.php';
?>
...
<div class="container">
    <table class="table table-bordered table-striped fs-6">
        ...
        <?php if(isset($data) && count($data) > 0):?>
            <?php foreach ($data as $item):?>
                <tr class="border">
                    <th scope="row"><img src="img/<?=htmlspecialchars($item['img_path']);?>" style="max-width: 200px"></th>
                    ...
                    <td>
                        <form method="POST" action="update.php">
                            <button  value="<?=$item['id'];?>" class='col-11 btn btn-primary m-2' name="change" type="submit">Изменить</button>
                        </form>
                        <form method="post" action="crud.php">
                            <button onclick="return confirm('Вы действительно хотите удалить запись?')" value="<?=$item['id'];?>" class='col-11 btn btn-danger m-2' name="delete" type="submit">Удалить</button>
                        </form>
                    </td>
                </tr>
            <?php endforeach;?>
    ...
    <a href="add.php"><button class="col-2 btn btn-primary">Добавить</button></a>
</div>
<div class="test">
