...
require_once $_SERVER['DOCUMENT_ROOT'] . '/ProjectsPHP/core/logic_login.php';
?>
    ...
    <div class="container">
        <div class="search">
            <div class="input-group">
                ...
                <input value="" type="search" class="form-control" placeholder="Поиск по анализам" id="ser" aria-label="Search" aria-describedby="search-addon" />
...
<div class="container">
    <div class="row">
        <p class="py-2"> <a href="analysis.php">Домашняя страница</a> &gt;  Вход в аккаунт </p>
    </div>
    <!-- Форма -->
    <div class="col-md-5 mx-auto">
        ...
        <form action="login.php" method="POST">
            ...
            <div class="form-group">
                ...
                <input value="<?php if(!empty($_POST['email'])) echo htmlspecialchars($_POST['email'])?>" type="email" name="email" class="form-control" placeholder="example@example.com">
                <span class="error"><?php if(!empty($emptyFields['email'])) echo htmlspecialchars($emptyFields['email']);?></span>
            </div>
            <div class="form-group">
                <label for="password">Пароль</label>
                <input value="<?php if(!empty($_POST['password'])) echo htmlspecialchars($_POST['password'])?>" type="password" name="password" id="password" class="form-control" placeholder="**********">
                <span class="error"><?php if(!empty($emptyFields['password'])) echo htmlspecialchars($emptyFields['password']);?></span>
            </div>
            <div class="col-md-12 text-center ">
                <button name="submit" type="submit" class=" btn btn-block mybtn btn-primary tx-tfm login-btn">Войти</button>
            </div>
            <div class="form-group">
                <p class="text-center">Ещё нет аккаунта? <a href="signup.php">Зарегистрируйтесь</a></p>
            </div>
            <span class="error"><?php if(!empty($error)) echo htmlspecialchars($error);?></span>
        </form>
    </div>
</div>
