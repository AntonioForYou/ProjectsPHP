...
require_once $_SERVER['DOCUMENT_ROOT'] . '/ProjectsPHP/core/logic_analysis.php';
?>
    ...
    <div class="container-fluid bg-dark">
        <nav class="navbar container justify-content-center flex-column">
            <div class="row justify-content-md-center">
                ...
                <div class="col-md-auto align-content-center">
                    <div class="head-link nav-link">Вы авторизированы как <?php echo UserLogic::current()?></div>
                </div>
                <div>
                    <div class="col-md-auto align-content-center">
                        <form action="analysis.php" method="post" id="logout">
                                <button name="logout" class="btn btn-dark">Выход</button>
                        </form>
                    </div>
                </div>
        </nav>
    ...
    <div class="row promo-banner">
        <div class="col-8">
            <div class="row row-cols-1">
                <div class="col">
                    <div class="promo-title text-white">
                        ПРОЙТИ
                        <br>
                        ДИАГНОСТИКУ
                        <br>
                ...
                <div class="col">
                    <div class="promo-subtitle text-white">
                        быстрее, чем думать могли ли
                        <br>
                        вы заразиться
                    </div>
                </div>
        ...
        <div class="col adv">
            <div class="promo-advantages">
                <div class="row-cols-1 str">
                    <div class="col cln">
                        <div class="element-adv row">
                            <div class="col-4">
                                <img src="icons/cov-advantage-1.svg" class="adv-img" alt="">
                            </div>
                            <div class="col">
                                Точно
                                <br>
                                <span>определяем все штаммы, в т.ч. Omicron и Delta </span>
                            </div>
                        </div>
                    </div>
                    <div class="col cln">
                        <div class="element-adv row">
                            <div class="col-4">
                                <img src="icons/cov-advantage-2.svg" class="adv-img" alt="">
                            </div>
                            <div class="col">
                                Быстро
                                <br>
                                <span>результат от 1 дня</span>
                    ...
                    <div class="col cln">
                        <div class="element-adv row">
                            <div class="col-4">
                                <img src="icons/cov-advantage-3.svg" class="adv-img" alt="">
                            </div>
                            <div class="col">
                                Удобно
                                <br>
                                <span>Сдать анализ можно бесконтактным способом</span>
                            </div>
                        </div>
                    </div>
                </div>
...
<div class="container">
    <form action="analysis.php" method="get" class="test-content">
        <label>Фильтрация результата поиска</label>
        <div class="mb-3">
            <label>По цене:</label>
            <input type="number" name="costFrom" placeholder="Цена от" value="<?php if(!empty($arBinds['costFrom'])) echo htmlspecialchars($arBinds['costFrom']);?>" class="form-control">
            <input type="number" name="costTo" placeholder="Цена до" value="<?php if(!empty($arBinds['costTo'])) echo htmlspecialchars($arBinds['costTo']);?>" class="form-control mt-3">
        </div>
        <div class="mb-3">
            <label>Фильтрация по заболеванию:</label>
            <select name="disease" class="form-control">
                <option value="">Выберите вид заболевания</option>
                <?php foreach ($disease as $item):?>
                    <option <?php if(!empty($arBinds['disease']) && $arBinds['disease'] == $item['id'])
        ...
        <div class="mb-3">
            ...
            <input type="text" name="description" placeholder="Введите описание"
                   value="<?php if(!empty($arBinds['description'])) echo htmlspecialchars($arBinds['description']);?>" class="form-control">
        </div>
        <div class="mb-3">
            <label>Фильтрация по анализу:</label>
            <input type="text" name="analysis" placeholder="Введите наименование анализа"
                   value="<?php if(!empty($arBinds['analysis'])) echo htmlspecialchars($arBinds['analysis']);?>" class="form-control">
        </div>
        <div class="row">
            <div class="col"><input type="submit" name="submit" value="Применить фильтр" class="btn btn-primary"></div>
            <div class="col"></div>
            <div class="col"></div>
            <div class="col"><input type="submit" name="clearFilter" value="Очистить фильтр" class="btn btn-danger float-right"></div>
        </div>
    </form>
</div>
<div class="container mt-3">
    <?php if(count($fullList) > 0):?>
        <table class="table">
            <thead>
                <tr>
                    <th class="test border" scope="col">Изображение</th>
                    <th class="test border" scope="col">Наименование</th>
                    <th class="test border" scope="col">Вид заболевания</th>
                    <th class="test border" scope="col">Описание</th>
                    <th class="test border" scope="col">Стоимость</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($fullList as $item):?>
                    <tr class="border">
                        <th scope="row"><img src="img/<?=htmlspecialchars($item['img_path']);?>" style="max-width: 250px"></th>
                        <td><?=htmlspecialchars($item['name']);?></td>
                        <td><?=htmlspecialchars($item['diseaseName']);?></td>
                        <td><?=htmlspecialchars($item['description']);?></td>
                        <td class="test"><?=htmlspecialchars($item['cost']);?></td>
                    </tr>
                <?php endforeach;?>
            </tbody>
        </table>
